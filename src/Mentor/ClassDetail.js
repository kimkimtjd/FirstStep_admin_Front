...
function ClassDetail() {
    ...
    useEffect(() => {
        fetch(`/api/admin/Class/detail/${location.pathname.split('/')[3]}`, {
            method: 'GET',
        })
            .then(response => {
                ...
                <Topinner>
                    ...
                    <FourthBox>
                        <div style={{ display: "flex", flexDirection: "row", alignItems: "flex-end" }}>
                            <span style={{ color: "#515151", fontSize: "16px", fontWeight: "600" }}>과목</span>
                        </div>

                        {data[0]?.Category2.split('-').map((data, index) => (
                            <div key={index}>
                                <Filterinput
                                    type="program"
                                    value={data}
